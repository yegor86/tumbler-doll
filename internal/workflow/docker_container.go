...
func AttachContainer(ctx context.Context, containerId string, command string) ([]byte, error) {
	// Create a Docker client
	docker, err := dockerClient.NewClientWithOpts(dockerClient.FromEnv, dockerClient.WithVersion(dockerClientVersion))
	if err != nil {
		return nil, fmt.Errorf("failed to create Docker client: %w", err)
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create exec instance for command '%v': %w", command, err)
	}

	// Start the command execution
	execAttachResp, err := docker.ContainerExecAttach(ctx, execResp.ID, container.ExecStartOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to start exec instance for command '%v': %w", command, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read output for command '%v': %w", command, err)
	}
